...
Diagram 2: API Management with circuit breaker in action.  
...
> **Important**: Update the names of the backend services in the [deploy.bicep](/deploy.bicep) file before running the next command.  
...
3. Click on Add API.
4. Select 'HTTP' API.
5. Give it a name and set the URL suffix to 'openai'.
...
4. Replace the existing policy with [this policy](/policy.xml) (showed below).
5. Click on 'Save'.
